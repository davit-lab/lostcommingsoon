...
const LostLockSite: React.FC = () => {
  ...
  // Preload background image, then fade it in via body[data-bg-loaded]
  useEffect(() => {
    const url = assets.mainBackground;
    if (!url) {
      document.body.removeAttribute('data-bg-loaded');
    ...
    img.onerror = () => {
      // Fall back silently — keep solid background color
      document.documentElement.style.removeProperty('--bg-url');
      document.body.removeAttribute('data-bg-loaded');
    };
    img.src = url;
