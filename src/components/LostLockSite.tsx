...
const LanguageSwitcher = ({ currentLang, setLang }: { currentLang: Language; setLang: (l: Language) => void }) => (
  <div className="flex items-center gap-2 bg-zinc-900/60 backdrop-blur-sm p-1 rounded-full border border-zinc-700/50">
    {(['ka', 'en'] as Language[]).map((code) => (
      <button key={code} onClick={() => setLang(code)}
        className={`px-3 py-1 rounded-full text-[10px] font-black transition-all ${currentLang === code ? 'bg-primary/10 text-primary border border-primary/20' : 'text-muted-foreground hover:text-foreground'}`}
...
const Nav = ({ onNavigate, lang, setLang }: { onNavigate: (hash: string) => void; lang: Language; setLang: (l: Language) => void }) => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full h-16 md:h-20 z-50 bg-black/70 backdrop-blur-md border-b border-zinc-800/50 flex items-center justify-between px-4 md:px-14">
      ...
      <div className="hidden lg:flex items-center gap-8">
        {t.nav.map((link: any) => (
          <button key={link.label} onClick={() => onNavigate(link.href)}
            className="text-[11px] font-black uppercase tracking-[0.3em] text-foreground/90 hover:text-primary transition-all"
          >{link.label}</button>
        ))}
      </div>
      <div className="hidden lg:flex items-center gap-6">
        <LanguageSwitcher currentLang={lang} setLang={setLang} />
        <button onClick={() => onNavigate('#book')} className="px-8 py-3 rounded-2xl bg-primary text-primary-foreground border border-primary/30 text-[10px] font-black hover:opacity-90 transition-all uppercase tracking-[0.2em]">{t.ui.book}</button>
      </div>
      <div className="lg:hidden flex items-center gap-3">
        <LanguageSwitcher currentLang={lang} setLang={setLang} />
        <button className="p-2 text-primary bg-zinc-900/60 backdrop-blur-sm rounded-lg border border-zinc-800/50" onClick={() => setIsOpen(!isOpen)}>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
            className="absolute top-[72px] left-4 right-4 bg-zinc-900/60 backdrop-blur-sm rounded-2xl border border-zinc-800/50 p-8 flex flex-col gap-6 lg:hidden shadow-xl z-50">
            {t.nav.map((link: any) => (
            ...
            <button onClick={() => { onNavigate('#book'); setIsOpen(false); }}
              className="mt-4 w-full py-5 bg-primary border border-primary/30 rounded-2xl text-center text-primary-foreground font-black uppercase text-xs hover:opacity-90 transition-all">{t.ui.book}</button>
          </motion.div>
        )}
      </AnimatePresence>
    </nav>
  ...
  return (
    <motion.div whileHover={{ y: -12 }} onClick={onClick} className="group flex flex-col gap-4 md:gap-8 cursor-pointer w-full relative">
      <div className="relative aspect-square overflow-hidden rounded-[24px] md:rounded-[32px] border border-zinc-800/50 bg-zinc-900/40 backdrop-blur-sm shadow-xl">
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-zinc-900/60 backdrop-blur-sm px-5 md:px-6 py-4 md:py-5 flex justify-between items-center">
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] font-black text-primary uppercase tracking-[0.4em]">{item.category}</span>
            <h3 className="text-lg md:text-xl font-black text-white tracking-tight uppercase leading-none">{item.name}</h3>
          </div>
          <div className="p-2.5 rounded-full bg-transparent text-primary border border-primary/30 shrink-0 group-hover:rotate-45 transition-transform duration-500">
...
const LostLockSite: React.FC = () => {
        ...
        <div className="flex flex-col items-center py-12 md:py-24 px-4 md:px-14 max-w-[1400px] mx-auto gap-12 md:gap-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-start">
            ...
            <div className="flex flex-col gap-8 md:gap-12 lg:sticky lg:top-32">
              ...
              <div className="p-8 md:p-10 rounded-[24px] md:rounded-[32px] bg-zinc-900/60 backdrop-blur-sm border border-zinc-800/50 shadow-xl">
                <p className="text-lg md:text-2xl text-foreground leading-relaxed font-bold">{itemMatch.description}</p>
              </div>
               <button onClick={() => navigateTo('#book')} className="py-5 md:py-7 rounded-[24px] md:rounded-[32px] bg-primary text-primary-foreground border border-primary/30 font-black text-lg md:text-2xl hover:opacity-90 transition-all uppercase tracking-widest">{t.ui.book}</button>
            </div>
          </div>
        </div>
      );
          ...
          <div className="py-12 md:py-24 px-4 md:px-14 max-w-[1400px] mx-auto min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mt-12">
              {t.pricing.map((pkg: any, idx: number) => (
                <div key={idx} className={`p-8 md:p-10 rounded-[24px] flex flex-col gap-6 border shadow-xl ${pkg.recommended ? 'bg-zinc-900/60 backdrop-blur-sm border-primary/60' : 'bg-zinc-900/60 backdrop-blur-sm border-zinc-800/50'}`}>
                  {pkg.recommended && <span className="bg-transparent text-primary border border-primary/30 px-4 py-1 rounded-full text-[10px] font-black uppercase w-fit">★</span>}
                  ...
                  <button onClick={() => navigateTo('#book')} className="mt-auto py-4 rounded-2xl bg-transparent text-primary border border-primary/30 font-black uppercase text-xs hover:bg-primary/10 transition-all">{t.ui.book}</button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="py-12 md:py-24 px-4 md:px-14 max-w-[1400px] mx-auto min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 mt-12">
              {Object.entries(t.menu).map(([key, cat]: [string, any]) => (
                <div key={key} className="p-8 md:p-10 rounded-[24px] bg-zinc-900/60 backdrop-blur-sm border border-zinc-800/50 shadow-xl flex flex-col">
                  <h2 className="text-2xl md:text-3xl font-black tracking-tighter uppercase mb-10 text-primary">{cat.title}</h2>
                  <div className="flex flex-col gap-4">
                    {cat.items.map((item: any, i: number) => (
                      <div key={i} className="flex justify-between items-center py-4 border-b border-zinc-800 last:border-0 hover:bg-zinc-800 px-2 rounded-xl transition-colors">
          ...
          <div className="py-12 md:py-24 px-4 md:px-14 max-w-[1400px] mx-auto min-h-screen flex flex-col items-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 w-full items-start">
              <div className="flex flex-col gap-6 md:gap-8">
                ...
                <a href="tel:+995568967277" className="p-8 md:p-10 rounded-[32px] md:rounded-[48px] bg-transparent border border-primary/30 flex items-center justify-between group hover:bg-primary/10 transition-all">
                  ...
                  <ArrowIcon size={32} className="text-primary/30 hidden md:block" />
                </a>
              </div>
          ...
          <div className="py-12 md:py-24 px-4 md:px-14 max-w-[900px] mx-auto min-h-screen">
            ...
            <div className="flex flex-col gap-4 md:gap-6 mt-12">
              {t.faq.map((item: any, idx: number) => (
                <details key={idx} className="group border border-zinc-800/50 rounded-2xl bg-zinc-900/60 backdrop-blur-sm overflow-hidden shadow-xl">
          ...
          <div className="py-12 md:py-24 px-4 md:px-14 max-w-[1000px] mx-auto min-h-screen">
            ...
            <div className="flex flex-col gap-6 mt-12">
              {t.rules.map((rule: string, idx: number) => (
                <div key={idx} className="p-8 md:p-10 rounded-[24px] bg-zinc-900/60 backdrop-blur-sm border border-zinc-800/50 flex gap-8 items-start hover:bg-zinc-800/60 transition-all shadow-xl">
          ...
          <div className="flex flex-col items-center gap-16 md:gap-32 pb-16 md:pb-32 px-4 md:px-0">
            ...
            <section className="w-full max-w-[1400px] md:px-14 flex flex-col gap-8 md:gap-16">
              <div className="flex justify-between items-end">
                ...
                <button onClick={() => navigateTo('#obstacles')} className="text-[10px] font-black uppercase text-primary hover:text-foreground flex items-center gap-3 bg-zinc-900/60 backdrop-blur-sm px-6 py-3 rounded-full border border-zinc-700/50 transition-all">{t.ui.seeAll} <ChevronRight size={16} /></button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
                {t.obstacles.slice(0, 3).map((item: any) => <CardComp key={item.id} item={item} lang={lang} onClick={() => navigateTo(item.link)} />)}
              </div>
            </section>
            <div className="w-full h-px bg-zinc-800 max-w-[1400px]" />
            <section className="w-full max-w-[1400px] md:px-14 flex flex-col gap-8 md:gap-16">
              <div className="flex justify-between items-end">
                <h2 className="text-4xl md:text-8xl font-vintage text-foreground uppercase leading-none">{t.ui.services}</h2>
                <button onClick={() => navigateTo('#services')} className="text-[10px] font-black uppercase text-primary hover:text-foreground flex items-center gap-3 bg-zinc-900/60 backdrop-blur-sm px-6 py-3 rounded-full border border-zinc-700/50 transition-all">{t.ui.seeAll} <ChevronRight size={16} /></button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
  ...
  return (
    ...
    >
      ...
      <footer className="w-full py-16 md:py-32 px-4 md:px-14 border-t border-zinc-800 bg-black mt-auto relative z-10">
