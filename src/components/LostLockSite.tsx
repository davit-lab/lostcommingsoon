...
const LanguageSwitcher = ({ currentLang, setLang }: { currentLang: Language; setLang: (l: Language) => void }) => (
  <div className="flex items-center gap-2 bg-zinc-900 p-1 rounded-full border border-zinc-700">
...
const Nav = ({ onNavigate, lang, setLang }: { onNavigate: (hash: string) => void; lang: Language; setLang: (l: Language) => void }) => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full h-16 md:h-20 z-50 bg-black border-b border-zinc-800 flex items-center justify-between px-4 md:px-14">
      ...
      <div className="lg:hidden flex items-center gap-3">
        <LanguageSwitcher currentLang={lang} setLang={setLang} />
        <button className="p-2 text-primary bg-zinc-900 rounded-lg border border-zinc-800" onClick={() => setIsOpen(!isOpen)}>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
            className="absolute top-[72px] left-4 right-4 bg-zinc-900 rounded-2xl border border-zinc-800 p-8 flex flex-col gap-6 lg:hidden shadow-xl z-50">
            {t.nav.map((link: any) => (
...
const CardComp: React.FC<{ item: ServiceItem; onClick: () => void; lang: Language }> = ({ item, onClick, lang }) => {
  ...
  return (
    <motion.div whileHover={{ y: -12 }} onClick={onClick} className="group flex flex-col gap-4 md:gap-8 cursor-pointer w-full relative">
      <div className="relative aspect-square overflow-hidden rounded-[24px] md:rounded-[32px] border border-zinc-800 bg-zinc-900 shadow-xl">
        ...
        <button onClick={handleShare} className="absolute top-4 right-4 z-20 p-2.5 rounded-full bg-zinc-900 border border-zinc-800 text-primary hover:bg-primary hover:text-primary-foreground transition-all">
          <AnimatePresence mode="wait">
            {copied ? <motion.div key="check" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}><Check size={15} /></motion.div> : <Share2 size={15} />}
          </AnimatePresence>
        </button>
        <div className="absolute bottom-0 left-0 right-0 bg-zinc-900 px-5 md:px-6 py-4 md:py-5 flex justify-between items-center">
          ...
          <div className="p-2.5 rounded-full bg-primary text-primary-foreground shrink-0 group-hover:rotate-45 transition-transform duration-500">
...
const LostLockSite: React.FC = () => {
      ...
      return (
        <div className="flex flex-col items-center py-12 md:py-24 px-4 md:px-14 max-w-[1400px] mx-auto gap-12 md:gap-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-start">
            <div className="relative aspect-square rounded-[24px] md:rounded-[40px] overflow-hidden border border-zinc-800 bg-zinc-900 shadow-xl">
            ...
            <div className="flex flex-col gap-8 md:gap-12 lg:sticky lg:top-32">
              ...
              <div className="p-8 md:p-10 rounded-[24px] md:rounded-[32px] bg-zinc-900 border border-zinc-800 shadow-xl">
          ...
          <div className="py-12 md:py-24 px-4 md:px-14 max-w-[1400px] mx-auto min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mt-12">
              {t.pricing.map((pkg: any, idx: number) => (
                <div key={idx} className={`p-8 md:p-10 rounded-[24px] flex flex-col gap-6 border shadow-xl ${pkg.recommended ? 'bg-zinc-900 border-primary' : 'bg-zinc-900 border-zinc-800'}`}>
          ...
          <div className="py-12 md:py-24 px-4 md:px-14 max-w-[1400px] mx-auto min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 mt-12">
              {Object.entries(t.menu).map(([key, cat]: [string, any]) => (
                <div key={key} className="p-8 md:p-10 rounded-[24px] bg-zinc-900 border border-zinc-800 shadow-xl flex flex-col">
          ...
          <div className="py-12 md:py-24 px-4 md:px-14 max-w-[900px] mx-auto min-h-screen">
            ...
            <div className="flex flex-col gap-4 md:gap-6 mt-12">
              {t.faq.map((item: any, idx: number) => (
                <details key={idx} className="group border border-zinc-800 rounded-2xl bg-zinc-900 overflow-hidden shadow-xl">
          ...
          <div className="py-12 md:py-24 px-4 md:px-14 max-w-[1000px] mx-auto min-h-screen">
            ...
            <div className="flex flex-col gap-6 mt-12">
              {t.rules.map((rule: string, idx: number) => (
                <div key={idx} className="p-8 md:p-10 rounded-[24px] bg-zinc-900 border border-zinc-800 flex gap-8 items-start hover:bg-zinc-800 transition-all shadow-xl">
          ...
          <div className="flex flex-col items-center gap-16 md:gap-32 pb-16 md:pb-32 px-4 md:px-0">
            ...
            <section className="w-full max-w-[1400px] md:px-14 flex flex-col gap-8 md:gap-16">
              <div className="flex justify-between items-end">
                ...
                <button onClick={() => navigateTo('#obstacles')} className="text-[10px] font-black uppercase text-primary hover:text-foreground flex items-center gap-3 bg-zinc-900 px-6 py-3 rounded-full border border-zinc-700 transition-all">{t.ui.seeAll} <ChevronRight size={16} /></button>
              </div>
            ...
            <section className="w-full max-w-[1400px] md:px-14 flex flex-col gap-8 md:gap-16">
              <div className="flex justify-between items-end">
                ...
                <button onClick={() => navigateTo('#services')} className="text-[10px] font-black uppercase text-primary hover:text-foreground flex items-center gap-3 bg-zinc-900 px-6 py-3 rounded-full border border-zinc-700 transition-all">{t.ui.seeAll} <ChevronRight size={16} /></button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
  ...
  return (
    <div className="min-h-screen relative flex flex-col selection:bg-primary selection:text-primary-foreground overflow-x-hidden">
